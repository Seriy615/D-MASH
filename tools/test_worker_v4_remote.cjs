...
(async()=>{
 ...
 try{
  ...
  for(const name of ['node_identity.js','node_runtime_host_v4.js'])await page.addScriptTag({url:new URL('js/'+name,base).href});
  const result=await page.evaluate(async({url,nodeId})=>{
   if(Core.keys)throw Error('Account unexpectedly unlocked');
   let identity;
   ...
   return {connected:true,reconnected:true,locked:!DeviceRoot.state,noAccount:!Core.keys};
  },{url,nodeId});
