...
/*
 * D-MASH browser acceptance fixes v53.
 *
 * Emergency local recovery wipe:
 *   0010<MASTER_KEY>=
 *
 * The 0010 prefix is captured from raw calculator key presses because the
 * decoy display intentionally normalizes leading zeroes. The suffix must match
 * the configured Master key verifier before any destructive action begins.
 * The sequence exists only in RAM and is cleared on AC/operators/evaluation.
 */
(function installAcceptanceV53(global) {
    const UI_PATCH = "__dmashAcceptanceV53EmergencyWipe";
    const PREFIX = "0010";
    ...
    async function deleteDatabase(name) {
        ...
        await new Promise(resolve => {
            ...
            try {
                ...
                // Another D-MASH tab can temporarily hold a database open. Keep
                // the delete request alive, but do not strand the emergency UI.
    ...
    async function wipeOriginStorage() {
        ...
        // Queue database deletion before removing the rest of the origin state.
        ...
        // Expire script-visible cookies for the current origin. HttpOnly cookies
        // cannot be removed by browser JavaScript and D-MASH must not rely on
        // them for local identity state.
        try {
    ...
    async function emergencyWipe(uiObject, masterKey) {
        ...
        // replace(), not assign(): do not add another D-MASH history entry.
    ...
    function patchUi(uiObject) {
        ...
        uiObject.num = function emergencySequenceNumberV53(token) {
        ...
        uiObject.cmd = function emergencySequenceCommandV53(command) {
            // The reserved sequence is contiguous digits only. Any calculator
            // operation or AC exits the emergency recognizer immediately.
            this._emergencyWipeSequence = "";
            return originalCmd(command);
        };

        uiObject.eval = async function emergencySequenceEvalV53(...args) {
            if (this.mode === 0) {
