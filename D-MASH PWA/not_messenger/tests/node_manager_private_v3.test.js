...
(async () => {
    ...
    const sourceRoute = {routeId: 'source-route'};
    DeviceRoutes.resolve = route => route === sourceRoute.routeId ? sourceRoute : null;
    let statusReads = 0, activation;
    NodeManager.routeStatus = async () => (++statusReads > 1 ? {connection, result: {hop_route_label: 'ad'.repeat(32)}} : null);
    NodeManager.activatePublicRouteOnConnection = async (route, node, options) => {activation = {route, node, options};};
    const ready = await NodeManager.ensurePublicRouteV3(certificate.routeId, sourceRoute.routeId);
    assert.equal(ready.connection, connection);
    assert.equal(activation.route, sourceRoute);
    assert.equal(activation.options.targetRoute, certificate.routeId, 'unknown public target gets an explicit authorized Probe');
    NodeManager.routeStatus = async () => null;
    assert.equal(await NodeManager.ensurePublicRouteV3(certificate.routeId, sourceRoute.routeId, 0), null, 'missing destination remains unavailable without SUBMIT fallback');
    a.close();
